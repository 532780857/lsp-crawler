...
import com.googlevip8.lspCrawler.utils.ConsoleProgressBar;
...
public class M3u8OneDownloadFactory {
    ...
    public class M3u8Download {
        ...
        //优化内存占用
        private final BlockingQueue<byte[]> BLOCKING_QUEUE = new LinkedBlockingQueue<byte[]>();
        ...
        //所有ts片段下载链接
        private Set<String> tsSet = new LinkedHashSet<String>();

        //解密后的片段
        private Set<File> finishedFiles = new ConcurrentSkipListSet<File>(Comparator.comparingInt(o -> Integer.parseInt(o.getName().replace(".xyz", ""))));
...
//                Log.i("不需要解密");
        ...
        private void startDownload() {
            ...
            downMonitor(fixedThreadPool);

            startListener(fixedThreadPool);

            isTerminated(fixedThreadPool);
         ...
         * 是否等待下载完成之后继续
         */
        public void isTerminated(ExecutorService fixedThreadPool){
...
//                Log.i("视频合并完成，欢迎使用!");
         ...
         */
        private void downMonitor(ExecutorService fixedThreadPool) {
            new Thread(() -> {
                int consume = 0;
                //轮询是否下载成功
                while (!fixedThreadPool.isTerminated()) {
                    try {
                        ...
                        Thread.sleep(1000L);
                        if (tsSet.size() > 0) {
                            BigDecimal bigDecimal1 = new BigDecimal(finishedCount).divide(new BigDecimal(tsSet.size()), 4, BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(100)).setScale(2, BigDecimal.ROUND_HALF_UP);
                            ConsoleProgressBar consoleProgressBar = new ConsoleProgressBar();
                            consoleProgressBar.show(bigDecimal1.longValue());
...
//                Log.i("下载完成，正在合并文件！共" + finishedFiles.size() + "个！" + StringUtils.convertToDownloadSpeed(downloadBytes, 3));
        ...
        private void deleteFiles() {
            File file = new File(dir+FILESEPARATOR+"tmp");
            if (file.exists()){
        ...
        private Thread getThread(String urls, int i) {
            return new Thread(() -> {
                ...
                String tmp = dir + FILESEPARATOR +"tmp";
                String xyFilePath= tmp + FILESEPARATOR + i + ".xy";
                String xyzFilePath= tmp + FILESEPARATOR + i + ".xyz";
                //xy为未解密的ts片段，如果存在，则删除
                File file2 = new File(xyFilePath);
                if (file2.exists()) {
                    file2.delete();
                }
                ...
                while (count <= retryCount) {
                    try {
                        ...
                        File file = new File(xyzFilePath);
...
//                    e.printStackTrace();
        ...
        private void checkField() {
            if ("m3u8".compareTo(MediaFormat.getMediaFormat(DOWNLOADURL)) != 0)
                throw new M3u8Exception("不是一个完整m3u8链接！\n"+DOWNLOADURL);
            if (threadCount <= 0)
